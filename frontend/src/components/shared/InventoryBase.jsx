...
const InventoryBase = ({ 
      ...
      if (filters.name) {
        ...
        if (activo.responsable) {
          if (typeof activo.responsable === 'string') {
            matchesResponsible = activo.responsable.toLowerCase().includes(searchTerm);
          } else if (activo.responsable.nombreCompleto) {
            matchesResponsible = activo.responsable.nombreCompleto.toLowerCase().includes(searchTerm);
          }
        }
      ...
      <div className={`inventory-page ${className}`} {...props}>
        <div className="user-header">
          <div className="user-header-text">
            <h2>{getRoleTitle()}</h2>
            <h6>{role === 'usuario' ? 'Mis activos asignados' : 'Cargando...'}</h6>
          </div>
        </div>
  ...
  return (
    <div className={`inventory-page ${className}`} {...props}>
      ...
      {activosToShow.length === 0 && !loading && !error && role === 'usuario' ? (
        <div className="alert alert-info text-center">
          <h5>Sin activos asignados</h5>
          <p>No tienes activos asignados actualmente. Puedes crear uno nuevo usando el botón superior.</p>
        </div>
      ) : (
          ...
          {activosToShow.map((activo, index) => {
            ...
            return (
              <CardActivo
                ...
                onModificarClick={role === 'usuario' ? handleModificarClick : undefined}
